...
```{r yhat mlcar, echo = FALSE, message = FALSE, warning = FALSE, output = FALSE}

ybound <- c(0, (max(cav_MLCAR_inla$summary.fitted.values$mean)+1))

ggyhat21 <- dd_con %>% 
  dplyr::mutate(yhat_21 = round(cav_MLCAR_inla$summary.fitted.values$mean[c(1:n)],0)) %>% 
  ggplot2::ggplot() +
...
ggyhat22 <- dd_con %>% 
  dplyr::mutate(yhat_22 = round(cav_MLCAR_inla$summary.fitted.values$mean[c(n+(1:n))], 0)) %>% 
  ggplot2::ggplot() +
...
ggyhat23 <- dd_con %>% 
  dplyr::mutate(yhat_23 = round(cav_MLCAR_inla$summary.fitted.values$mean[c((2*n)+(1:n))],0)) %>% 
  ggplot2::ggplot() +
...
We show the posterior median of the marginal variances and correlations of $z$, i.e. $\Lambda^{-1}$:
```{r Sigma LCAR, message = FALSE, warning = FALSE}
Sigma_diagonal <- sapply(c(2,3,4), function(i){
  inla.zmarginal(inla.tmarginal(fun = function(x) {
    exp(-x)
    },  marginal =  cav_MLCAR_inla$marginals.hyperpar[[i]]),
    silent = T)$quant0.5
})
...
In terms of point estimates, the only high correlation appears that of 2021-2022; to have more insight on the dependence structure in $z$ we show some posterior summaries for the covariances:
...
xi_ls <- inla.zmarginal(inla.tmarginal(
  fun = function(X) 1/(1 + exp(-X)),
  marginal = cav_MLCAR_inla$marginals.hyperpar[[1]]), silent = T)


hyperpar_mat <- rbind(
  unlist(xi_ls[c(1,3,5,7)]),
...
#### Future developments: the BYM model

A possible improvement on the modelistic side would be implementing the BYM model [@BYM2]
...
Where $\phi$ is a mixing parameter (this rather crude simplification of employing a unique mixing parameter for all variables could be relaxed at a later stage), $\mathrm{vec}(u) \sim N(0, I_p \otimes L_{\mathrm{scaled}})$ is an ICAR latent field and $L_\mathrm{scaled}$ is the scaled Laplacian matrix, i.e. the Laplacian matrix multiplied by the graph typical variance in order to have the geometric mean of diagonal entries equal to $1$, and $v$ is an IID multivariate Standard Normal vector, such that $\mathrm{vec} (v) \sim N(0, I_p \otimes I_n)$. The matrix $M :/ M'M = \Lambda^{-1}$ is a generic full-rank matrix which does not necessarily need to be the Cholesky factor of the scale parameter; in fact, a convenient but not unique way to define it may be $M = D^{-\frac{1}{2}}E^{\top}$ where $E$ and $D$ are the eigenvector and eigenvalues matrices of $\Lambda$ [@Urdangarin]. 
...
A problematic element of the BYM model is that the precision matrix of $z$ is sparse, being it defined as $\mathrm{prec}[z] = \left( \phi \Lambda \otimes L^{+} + (1-\phi) \Lambda \otimes I_n \right)^{-1}$.
...
The problem is that to do so we need to define a latent field of length $2 np$, i.e. twice the observational length. Only the first half of this latent effect actually enters the linear predictor, still implementing such a reparametrisation in INLA is challenging.

Therefore, for the time being we rely on the classic, dense parametrisation, which is however very inefficient in computational terms.
...
```
```{r load mbym, echo = FALSE, warning = FALSE}
load("input/BYM_dense_uniquePhi.RData")
```

The fitting of the BYM model, though we used the simplest formulation,is slightly better than the LCAR:

```{r new WAICS, results = "asis", echo = FALSE, warning = FALSE, message = FALSE}
WAICS_0 <- WAICS
WAICS <- data.frame(
  Model = c(WAICS_0$Model, "BYM"),
  WAIC = c(WAICS_0$WAIC, cav_MBYM_inla$waic$waic),
  Eff_params= c(WAICS_0$Eff_params, cav_MBYM_inla$waic$p.eff))
print(xtable::xtable(WAICS, digits = 3), include.rownames = F)
```

The estimated mixing parameter, however, is unexpectedly high:

```{r mixing BYM, echo = FALSE}
...
cat("Posterior summaries of BYM mixing parameter: \n")
print(unlist(inla.zmarginal(phi_marg_bym, silent = TRUE)))
```

```{r Sigma BYM, message = FALSE, warning = FALSE, echo = FALSE}
Sigma_diagonal_bym <- sapply(c(2,3,4), function(i){
  inla.zmarginal(inla.tmarginal(fun = function(x) {
    exp(-x)
...
Sigma_offdiag_bym <- sapply(c(5, 6, 7), function(i){
  inla.zmarginal(inla.tmarginal(fun = function(x) {
    (2 * exp(x))/(1 + exp(x)) - 1
    }, marginal =  cav_MBYM_inla$marginals.hyperpar[[i]]),
    silent = T)$quant0.5
...
Sigma_bym <- Matrix::Diagonal(x = Sigma_diagonal_bym)
Sigma_bym[lower.tri(Sigma_bym)] <- Sigma_offdiag_bym
Sigma_bym[upper.tri(Sigma_bym)] <- t(Sigma[lower.tri(Sigma_bym)])
cat("Median of the BYM scale matrix: \n")
print(Sigma_bym)

```

The scale parameter instead is quite close to the LCAR result instead.
...
The most notable change is the shrinkage in the effect of employment rate in 2021 and 2023. This being said, the interpretation of the posteriors of $\beta$ is overall consistent with the base MLCAR model. 
