...
#print(alphas.nosp.inla, n.digits = 3)
...
resids_glm_21 <- log(dd_con$N_ACC_21) - log(cav_glm_21$fitted.values) 
resids_glm_22 <- log(dd_con$N_ACC_22) - log(cav_glm_22$fitted.values) 
resids_glm_23 <- log(dd_con$N_ACC_23) - log(cav_glm_23$fitted.values) 
...
In the following, the area-specific latent field is denoted as $z_i = (z_{i1}^{\top}\, z_{i2}^{\top}\, z_{i3}^{\top})^{\top}$
...
\begin{equation}
\eta_{it} = X_{it}^{\top} \alpha_t + z_{i} + \delta_{it}
\label{eq:st}
\end{equation}
...
alphas.MBYM.inla   <- cav_MBYM_inla$summary.fixed %>% 
  dplyr::select(.data$mean, .data$sd)  %>%
  dplyr::mutate( year = rep(c(2021, 2022, 2023), 8)) %>% 
...
#alphas.MBYM.inla <- dplyr::bind_rows(
...
#             Sd_2023 = cav_MBYM_inla$summary.fixed$sd[1]),
 # alphas.MBYM.inla.noint) 
#alphas.MBYM.inla.ci   <- cav_MBYM_inla$summary.fixed %>% 
#    dplyr::select(.data$mean, .data$`0.025quant`, .data$`0.975quant`)  %>%
#    dplyr::mutate( year = rep(c(2021, 2022, 2023), 8)) %>% 
#    dplyr::mutate(Effect = rep(c("Intercept", "TEP",
#                                 rownames(summary(cav_glm_21)$coefficients)[-c(1,2)]),each=3)) %>% 
#    dplyr::relocate(.data$Effect, .before = 1) %>%  
#    dplyr::rename(Mean = .data$mean, Q0.025 = .data$`0.025quant`,
#                  Q0.975 = .data$`0.975quant`) %>% 
#    tidyr::pivot_wider( 
#        id_cols = .data$Effect,            
#        names_from = .data$year,                
#        values_from = c(.data$Mean, .data$Q0.025, .data$Q0.975))
...
$$
\lambda_{it} = e^{\displaystyle{\alpha_{0t} + \alpha_{1t} X_{i1,t} + ... + \alpha_{pt} X_{ik,t}} + z_{it}}
$$
And
$$
\pi_{it} = \left[1 + e^{- \displaystyle{(\beta_{0t} + \beta_{1t} X_{k+1,t} + ... + \beta_{mt} X_{i (k+m),t})}} \right]^{-1}
$$
...
alphas.MBYM.inlabru  <- cav_MBYM_inlabru$summary.fixed %>% 
    dplyr::select(.data$mean, .data$sd)  %>%
    dplyr::mutate(effect = rownames(.)) %>% 
    tidyr::separate(.data$effect, into = c("e1", "e2", "year"), sep = "_") %>% 
    tidyr::unite("Effect", c(.data$e1, .data$e2), remove = T)%>% 
  dplyr::relocate(.data$Effect, .before = 1) %>%  
  dplyr::rename(Mean = .data$mean, Sd = .data$sd) %>% 
  tidyr::pivot_wider( 
    id_cols = .data$Effect,            
...
The changes in $E[\alpha|y]$ may suggest we are in presence of confounding bias. 
...
```{r alphas leroux spatplus, echo = FALSE, results = "asis", message = FALSE}
alphas.mbym.inla.1  <- cav_MBYM_inla_spatplus$summary.fixed %>% #[-1,] 
  dplyr::select(.data$mean, .data$sd)  %>%
  dplyr::mutate( year = rep(c(2021, 2022, 2023), 8)) %>% 
  dplyr::mutate(Effect = rep(c("Intercept", "TEP", rownames(summary(cav_glm_21)$coefficients)[-c(1,2)]),each=3)) %>% 
  dplyr::relocate(.data$Effect, .before = 1) %>%  
...
#alphas.mlcar.inla1 <- dplyr::bind_rows(
#  data.frame(Effect = "Intercept", 
...
#  alphas.mlcar.inla.noint1) 

print(xtable::xtable(alphas.mbym.inla.1, num.digits = 3), include.rowname = FALSE)

```

The most notable change is perhaps the shrinkage in the effect of employment rate in 2021 and 2023. This being said, the interpretation of the posteriors of $\alpha$ is overall consistent with the base MLCAR model. 
