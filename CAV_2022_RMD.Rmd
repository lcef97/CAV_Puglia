...
In `R`, $d$ is encoded as \texttt{\detokenize{TEP_th}}.

\item $\mathrm{AES}$, the distance from the closest infrastructural pole, always measured in travel time. It is encoded as `AES`


\item `MFI`, i.e. the decile of municipality vulnerability index.
\item `PDI`, i.e. the dependency index, i.e. population either $\leq 20$ or $ \geq 65$ years over population in $[20 - 64]$ years.
\item `ELL`, i.e. the proportion of people aged $[25-54]$ with low education.
\item `ER`, i.e. employment rate among people aged $[20-64]$.
...
cav_pcar_INLA_th <- inla(N_ACC ~ 1 + TEP_th + AES + 
