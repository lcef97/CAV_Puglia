...
hyperpars <- tibble::tibble(
  ...
  q0.025 = round(c(cav_bym_INLA$summary.hyperpar$`0.025quant`, cav_bym_INLA_ZIP$summary.hyperpar$`0.025quant`),3),
...
```{r zhat bym zip, echo = FALSE}
dd_con %>% 
  dplyr::mutate(zhat = cav_bym_INLA_ZIP$summary.random$ID$mean[c(1:nrow(dd_con))]) %>% 
  ggplot2::ggplot() +
