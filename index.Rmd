...
<!--
To this aim, an issue worthy of being explored is underreporting modelling. If we observe $y_i$ accesses to AVCs, the true number of VAW incidents can be defined as, say, $\chi_i$. A reasonable model would be the *pogit*, assuming that:

\begin{align*}
y_i \mid \chi_i \sim \mathrm{Bin}(\chi_i, \frac{e^{\gamma_i}}{1 +e^{ \gamma_i}})   \\
\chi_i \sim \mathrm{Poisson}(e^{\eta_i+P_i})
\end{align*}
And this model implies that, marginalising over the true number of VAW cases $\chi$,
$$
y_i \sim \mathrm{Poisson}(\frac{e^{\gamma_i}}{1 +e^{\gamma_i}} \, e^{\eta_i+P_i} )
$$
