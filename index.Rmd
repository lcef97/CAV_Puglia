...
As an exploratory tool, we rely on a Poisson-logistic model (Pogit, [@Pogit], [@PogitZIP]). In brief, we have
$$
...
R codes are based on the ones shared by [@Wollo] at \url{https://github.com/saraew/Prosjektoppgave}.
...
```{r inlabru, output = FALSE, message = FALSE}
...
cmp <- ~ 
  ...
  beta2(main = 1, model = "linear",
        mean.linear = 0,
        prec.linear = 1e-3)+ 
  bym(ID, model = "bym2", graph = Lapl_con, constr = T, scale.model = T,
          hyper = list(prec = list(prior = "pc.prec", param = c(1.5, 0.01)))) 
...
formula <- N_ACC   ~ 
  Intercept + myoffset + 
  ER +
  bym +  neglogexpit(v1 = TEP_th_22, v2 = ELL, beta0, beta1, beta2)
...
```{r inlabru summaries}
summary(cav_bym_INLABRU)
```


, since due to the respective domains of the two factors of the rate parameter we have $\lambda_i:= e^{\delta_i + P_i}$, with $\delta_i := \beta_0 + \beta_1 X_1 + ... + \beta_p X_p + z_i$  and $\pi_i := \mathrm{expit}(\gamma_i)$ with $\gamma_i := \beta_{p+1} + \beta_
...
To this aim, an issue worthy of being explored is underreporting modelling; a reasonable methodology would be *pogit* regression [@Pogit], [@PogitZIP]
