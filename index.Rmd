...
We should do the same for the other couple of variables but since they are not of great interest for the time being we will leave them bogh.

```{r forward selection, warnings = F}
covariates <- colnames(X)[-c(1, which(colnames(X)=="AES"))]
covs.in <- c()
BIC.min <- c()
...
```{r glm five covs}
summary(glm(
  as.formula(paste0("N_ACC ~ 1 +", paste(covs.in[c(1:6)], collapse = " + "))),
...
Interpreting the negative association with the density of productive units becomes harder - and do not us forget that association does not imply a causal relationship. Lastly, the association with population growth rate (fifth covariate) appears positive.
...
Hence e.g. each $14'6''$ of distance of the a given municipality from the closest support center are associated with a decrease of $0.28$ in the log-frequency at which women from that municipality access to support centers.
