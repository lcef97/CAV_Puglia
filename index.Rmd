...
Where $X$ are the covariate defined earlier, $\beta$ are covariate effects, and $P_i$ is the logarithm of female population aged $\geq 15$ in municipality $i$.
...
As an exploratory tool, we rely on a Poisson-logistic model (Pogit, see [@Pogit], [@PogitZIP]). In brief, we have
$$
y_i \sim \mathrm{Poisson(\lambda_i \pi_i)}
$$
Where $\lambda_i \in \mathbb{R}^{+}$ is the rate parameter controlling the occurrence of true counts, and $\pi_i \in [0,1]$ controls for reporting frequency. 
Due to the different domains of $\pi_i$ and $\lambda_i$, this parametrisation implies departure from linearity in the predictor $\eta_i:= \ln \lambda_i + \ln \pi_i - \ln P_i$.

For clarity, in the following we will denote $\alpha$ as the $k+1$ covariate effects entering $\lambda_i$ and $\beta$ the $m+1$ covariate effects entering $\pi_i$. We would thus have 
$$
\lambda_i = e^{\displaystyle{\alpha_0 + \alpha_1 X_{i1} + ... + \alpha_p X_{ik}}}
$$
And
$$
\pi_i = \left(1 + e^{- \displaystyle{(\beta_0 + \beta_1 X_{k+1} + ... + \beta_m X_{i, k+m})}} \right)^{-1}
$$
Hence the linear predictor is not linear anymore. 
...
We assign informative prior to the intercept $\beta_0$. The idea is that, when all other effects are zero, $\pi_i \approx 1/10$. 

However, interpreting $\beta_0$ is limited due to the mutual non-identifiability issue with $\alpha_0$. Hence, we must be happy with maintaining prior knowledge on $E[\beta_0]$.
...
```
This rather generic function allows to implement different models for $z$ at the cost of a couple of lines of code. Nevertheless, for the sake of brevity, we only fit the BYM model, which can be seen to perform slightly better than ICAR, PCAR and LCAR. The latter models can be simply fit by defining the analogues of `cmp_bym` through the `cmp_spatial()` wrapper.
...
cav_bru_basic <- function(model_cmp, verbose = FALSE) {
  terms <- c("alpha_0", "myoffset", "alpha_ELI", "alpha_PGR", "alpha_UIS",
             "alpha_PDI", "alpha_ER",
             paste0("neglogexpit(v1 = TEP_th_22, v2 = ELL,",
...
ggplot2::ggplot(betamarg_long, ggplot2::aes(x = .data$X, y = .data$Y, color = .data$Effect)) +
  ...
  ggplot2::labs(
    ...
    y = expression(pi(alpha ~ "|" ~ y) ~","~pi(alpha ~ "|" ~ y)),
    color = "Effect") +
  ggplot2::theme_classic()
...
betas_bru <- cbind(
  ...
  q0.975_ZIP = cav_bym_INLABRU$summary.fixed$`0.975quant`), 3))
