...
\item $d$, i.e. the distance of each municipality from the closest municipality hosting a support center. Distance is measured by road travel time in minutes.
For instance, the support center designated for the municipality of Adelfia (province of Bari, $3$rd municipality in the dataset) is located in Triggiano (BA). Then, $d_{3}$ denotes the travel time between Adelfia and Triggiano ($17$ minutes). 
...
\item $\mathrm{ELL}$, i.e. the proportion of people aged $[25-54]$ with low education.
\item $\mathrm{ERR}$, i.e. employment rate among people aged $[20-64]$.
\item $\mathrm{PGR}$, i.e. population growth rate with respect to 2011.
\item $\mathrm{UIS}$, i.e. the ventile of the density of local units of industry and services (where density is defined as the ratio between the counts of industrial units and population).
\item $\mathrm{ELI}$, i.e. the ventile of employees in low productivity local units by sector for industry and services.
