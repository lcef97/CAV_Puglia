...
dd_con %>% 
  dplyr::mutate(log_incidence_rate = log(.data$N_ACC) - log(.data$nn)) %>% 
  ggplot2::ggplot() +
  ggplot2::geom_sf(ggplot2::aes(fill = .data$log_incidence_rate))+
  #ggplot2::labs("Log incidence ratio") +
  ggplot2::scale_fill_viridis_c(name = "log-access rate", na.value = "white", direction = -1) +
...
cav_pcar_INLA<- inla(N_ACC ~ 1 +TEP_th_22 + ELI + PGR + UIS + ELL + PDI + ER + 
                           f(ID, model = PCAR.model(W = W_con, k = 1, lambda = log(100)/1.5)),
                         family = "poisson", offset = log(nn), data =dd_con,
...
#### Pogit model preliminary findings 

Model results are generally close to those of the linear Poisson regression (see previous section).
We compare covariates effects between the BYM and nonspatial model.  
...
alphas_bru <- cbind(
  ...
  round(tibble::tibble(
  Mean_BYM = cav_bym_inlabru$summary.fixed$mean,
  Mean_nosp = cav_nosp_inlabru$summary.fixed$mean,
  SD_BYM = cav_bym_inlabru$summary.fixed$sd,
  SD_nosp = cav_nosp_inlabru$summary.fixed$sd,
  q0.025_BYM = cav_bym_inlabru$summary.fixed$`0.025quant`,
  q0.025_nosp = cav_nosp_inlabru$summary.fixed$`0.025quant`,
  #Median = cav_bym_inlabru$summary.fixed$`0.5quant`,
  q0.975_BYM = cav_bym_inlabru$summary.fixed$`0.975quant`,
  q0.975_nosp = cav_nosp_inlabru$summary.fixed$`0.975quant`), 3))


alphas_bru
```
We also plot the posterior marginal densities of covariate effects under the BYM model.
```{r plot beta pogit, echo = FALSE, fig.cap = "Estimated covariate effects with the pogit model with BYM latent effect"}
