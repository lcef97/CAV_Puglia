...
Throughout this report, we assign $\Sigma$ a Wishart prior with 8 degrees of freedom and $I_4$ as the matrix-valued scale parameter. The choice of the degrees of freedom is based on the model allowing for the smallest WAIC; For the time being we only tested a number of degrees of freedom between 6 an 10. We also tried to assign an inverse-Wishart to $\Sigma$ with at least 8 degrees of freedom (otherwise it could not have finite moments) but the Wishart allows, yet again, for a smalle WAIC.
Tuning still needs to be done on the scale parameter. 
...
cav_MBYM_inla <- inla(
  N_ACC ~ 0 + Y_2021 + Y_2022 + Y_2023 + Y_2024 +
    TEP_th + ELI + PGR + UIS + ELL + PDI + ER+ 
    f(ID, model = inla.MBYM.Bartlett(k = 4, W = W_con, df = 8, PC = T ),
