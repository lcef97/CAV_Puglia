...
Z.true.M <-  U.true %*% diag(sqrt(phi.true)) %*% M.true + 
  V.true %*% diag(sqrt(1 - phi.true)) %*% M.true
Z.true <- as.vector(Z.true.M)
...
#' IMCAR model with spatiotemporal effect - should be the right
#' covariance-generating function, yet...

mod.IMCAR.AR1 <- inla(
  Y ~ 1 + X +
    f(ID, model = inla.IMCAR.AR1(k = 4, W = W, scale.model = T, sd.rate=5),
      extraconstr = list(A=constr.BYM$A[,-c(1:(k*n))], e = rep(0,k)) ),
  family = "poisson", data = dd, num.threads = 1, 
  control.inla = list(tolerance=1e-7, h=1e-5),
  control.compute = list(internal.opt = F, cpo = T, waic = T, config = T), 
...
do.call(rbind, lapply(c(1:k), function(i){
  inla.zmarginal(inla.tmarginal(fun=function(x) {
    exp(x)}, marginal = mod.IMCAR.AR1$marginals.hyperpar[[i]]), silent=T)
}) )
inla.zmarginal(inla.tmarginal(fun=function(x) 2*exp(x)/(1+exp(x))-1,
                              marginal = mod.IMCAR.AR1$marginals.hyperpar[[k+1]]))
...
#' Mixing parameter --> Messy
data.frame( do.call(rbind, lapply(
  lapply(mod.MMBYM.ST$marginals.hyperpar[1:k], function(f){
    inla.tmarginal(fun = function(X) 1/(1+exp(-X)), marginal = f)
  }), function(x) unlist(inla.zmarginal(x, silent = TRUE)))))[,c(1,2,3,5,7)]

#' Standard deviations --> OK
data.frame( do.call(rbind, lapply(
  lapply(mod.MMBYM.ST$marginals.hyperpar[k+1:k], function(f){
    inla.tmarginal(fun = function(X) exp(X), marginal = f)
  }), function(x) unlist(inla.zmarginal(x, silent = TRUE)))))[,c(1,2,3,5,7)]
...
inla.zmarginal(inla.tmarginal(
  ...
  marginal= mod.MMBYM.ST$marginals.hyperpar[[k+k+1]]))
