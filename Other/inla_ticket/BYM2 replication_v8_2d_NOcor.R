...
#' Hyperparameters ------------------------------------------------------------#
phi.true <- c(0.95, 0.4)
sigma.true <- c(1.2, 0.8)
#' Independent fields this time! ----------------------------------------------#
corr.true <- 0
...
dd <- data.frame(Y = y.sim, ID = c(1:(k*n)), group=rep(c(1:k), each=n))
...
mod.IMCAR <- inla(
  Y ~ 1 + 
    f(ID, model = inla.IMCAR.Bartlett(k = 2, W = W, df=4, scale.model = T ),
...
#' Correlations: low, indeed. This is good
vcov_summary(mod.IMCAR)$cor
 

#' BYM TOY example: independent fields ----------------------------------------#
#'
#' Ok this is literally the true DGP 
...
#' ...need a more strict prior on the mixing
mod.INDMMBYM  <- inla(
  Y ~ 1+
    f(ID, model = inla.INDMMBYM.model(k = 2, W = W, df=4, PC = T, alpha = 0.99, U = 0.5 ),
...
#' Variances ==> overestimated the first one
...
mod.MMBYM <- inla(
  Y ~ 1+ f(ID, model = inla.MMBYM.model(
    k = 2, W = W, df=4 ),
      extraconstr = constr.BYM),
  family = "poisson", data = dd, num.threads = 1,
...
mod.MMBYM.guided <- inla(
  Y ~ 1+  f(ID, model = inla.MMBYM.model(
    k = 2, W = W, df=4,    
    initial.values = c(rep(-3, 2), mod.IMCAR$summary.hyperpar$mode)),
      extraconstr = constr.BYM),
  family = "poisson", data = dd, num.threads = 1,
  #control.inla = list(tolerance = 1e-7),
  control.compute = list(internal.opt = F, cpo = T, waic = T, config = T), 
...
#' Variances: overestimated as usual
vcov_summary(mod.MMBYM.guided)
#' Mixing: overestimated
