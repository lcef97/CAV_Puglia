...
We compare the posteriors of $\alpha$ for the PCAR and BYM models. Starting from the PCAR
```{r alpha PCAR}
# \alpha_{1974}
...
For the determinant of the augmented BYM precision we would therefore have: $A = q_{11} \otimes I_n$; $B= q_{11} \tilde{\Phi} M^\top \otimes I_n$, $C = M \tilde{\Phi} q_11 \otimes I_n$ and $D = M \tilde{\Phi} q_{11}\tilde{\Phi}M^\top + I_p \otimes L$, hence 
$$
\mid D - CA^{-1}B \mid \, = \,
\mid M \tilde{\Phi} q_{11}\tilde{\Phi}M^\top + I_p \otimes L 
     -  M \tilde{\Phi} q_{11}\tilde{\Phi}M^\top \mid = \mid I_p \otimes L\mid = 0
$$
