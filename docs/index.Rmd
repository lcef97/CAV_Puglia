...
author: Leonardo Cefalo (UniBA), Alessio Pollice (UniBA), Crescenza Calculli (UniBA)
  and Diego Battagliese (UniBA)
date: "`r Sys.Date()`"
output:
...
$$\eta_{it} := X_{it}^\top \beta + Z_{it}$$
  ...
  - $Z_{it}$: multivariate latent spatial effect of size $n \times k$, where $n=256$ and $k=4 \quad \longrightarrow$ modelled via the flexible **M-Models** [@Mmodels]
...
#' It does not belong here but I would avoid
#' creating a chunk just to import the nonspatial model
#' so here it is
  load("WS/cav_nosp_inla.RData")
#' now here comes the ICAR
...
## M-models (I)
  ...
  - Based on the **decomposition** of the **variance-covariance** matrix as
  $\Sigma := M^\top M$
...
$$
\Omega_j = \left(\Lambda \otimes L + (I_k - \Lambda)  \otimes I_n
\right)^{-1}
$$
...
if(import_workspace){
  load("WS/cav_LMMCAR_inla_unif.RData")
  load("WS/cav_LMMCAR_inla_pc.RData")
} else {
...
cav_LMMCAR_inla_pc  <- inla(
  N_ACC ~  0+ Y_2021 + Y_2022 + Y_2023 + Y_2024 +
    TEP_th + ELI + PGR + UIS + ELL + PDI + ER+ 
    f(ID, model = inla.LMMCAR.model(k = 4, W = W_con, df = 8, PC = T, alpha=0.9, U = 0.6)),
  offset = log(nn),
  family = "poisson", data =dd,
  num.threads = 1, control.compute = list(
    internal.opt = F, cpo = T, waic = T, config = F), 
...
if(import_workspace){
  load("WS/cav_MMBYM_inla_unif.RData")
  load("WS/cav_MMBYM_inla_pc.RData")
} else {
  
cav_MMBYM_inla_pc <- inla(
  N_ACC ~ 0 + Y_2021 + Y_2022 + Y_2023 + Y_2024 +
    TEP_th + ELI + PGR + UIS + ELL + PDI + ER+ 
 ...
 **Penalise** the **departure** of a model depending on a parameter $\theta$ from a **simpler** (base) one [@PC]:
...
## PC priors: ractical findings
  ...
  - $\pi(\theta_j \mid \theta_1, \dots, \theta_{j-1}) = \pi(\theta_j)$, i.e., at each step $j$, 
  the prior of $\theta_j$ is independent on other parameters
...
<table style="width:100%">
  <tr>
    <td style="width:45%">
      <img src="https://raw.githubusercontent.com/lcef97/PhD_Thesis/main/docs/Media/PCAR_PC_prior.gif" width="440">
    </td>
    <td style="vertical-align: top; padding-left: 20px;">
    <!--
      <p><strong>PC Prior for PCAR autocorrelation parameter</strong></p>
      -->
      <ul>
        <li> Prior based on a regular lattice </li>  
        <li> Three examples for bound $\theta_0$: 0.4, 0.5, 0.6 </li>
        <li> Increasing tail probability $\mathcal{a} > \theta_0$ </li>
      </ul>
    </td>
  </tr>
</table>
...
<table style="width:100%">
  <tr>
    <td style="width:45%">
      <img src="https://raw.githubusercontent.com/lcef97/PhD_Thesis/main/docs/Media/LCAR_PC_prior.gif" width="440">
    </td>
    <td style="vertical-align: top; padding-left: 20px;">
    <!--
      <p><strong>PC Prior for PCAR autocorrelation parameter</strong></p>
      ...
      <ul>
        <li> Prior based on a regular lattice </li>  
        <li> Three examples for bound $\theta_0$: 0.5, 0.6, 0.7 </li>
        <li> Increasing tail probability $\mathcal{a} > \theta_0$ </li> 
        <li> Maybe better to work with **high** bound values and very high tail probabilities </li>
      </ul>
    </td>
  </tr>
</table>
...
<table style="width:100%">
  <tr>
    <td style="width:45%">
      <img src="https://raw.githubusercontent.com/lcef97/PhD_Thesis/main/docs/Media/BYM_PC_prior.gif" width="440">
    </td>
    <td style="vertical-align: top; padding-left: 20px;">
    <!--
      <p><strong>PC Prior for PCAR autocorrelation parameter</strong></p>
      -->
      <ul>
        <li> As in Riebler et al. (2016) </li
        <li> Prior based on a regular lattice </li>  
        <li> Three examples for bound $\theta_0$: 0.4, 0.5, 0.6 </li>
        <li> Increasing tail probability $\mathcal{a} > \theta_0$ </li>
      </ul>
...
Models compared by the WAIC -- **Watanabe--Akaike** Information Criterion [@GelmanWAIC]. Free parameters are reported as well. 

```{r all waic}

models <- list( cav_nosp_inla, cav_IMCAR_inla,
                cav_PMMCAR_inla_unif, cav_PMMCAR_inla_pc,
                cav_LMMCAR_inla_unif, cav_LMMCAR_inla_pc,
                cav_MMBYM_inla_unif, cav_MMBYM_inla_pc)

waics <- data.frame(t(sapply(models, function(x) {
  cbind(round(x$waic$waic, 2), round(x$waic$p.eff, 2))
  }))) %>%
  dplyr::rename(WAIC = .data$X1, P_WAIC = .data$X2) %>% 
  dplyr::mutate(Model = c("Nosp", "ICAR", "PCAR_Un", "PCAR_PC",
                          "LCAR_Un", "LCAR_PC", "BYM_Un", "BYM_PC")) %>% 
  dplyr::relocate(Model, .before = 1)

#dics <- data.frame(t(sapply(models, function(x) {
#  cbind(round(x$dic$dic, 2), round(x$dic$p.eff, 2))
#  }))) %>%
#  dplyr::rename(DIC = .data$X1, DIC_P_eff = .data$X2) 
...
## Model prior choice - BYM
  ...
  - Model estimation is carried out entirely within **R-INLA** [@INLA] using the **Variational Bayes** [@INLAVB] correction to the posterior mean of latent variables
...
```{r mixing param}
phi_post_pc <- Mmodel_compute_mixing(cav_MMBYM_inla_pc) %>% 
  dplyr::transmute(Year = c("2021", "2022", "2023", "2024"),
                   Mean = round(.data$mean, 3),
                   Q_0.025 = round(.data$quant0.025,3),
                   Median = round(.data$quant0.5,4),
                   Q_0.975 = round(.data$quant0.975,3))
rownames(phi_post_pc) <- NULL
DT::datatable(phi_post_pc)
```
...
```{r vcov param}
sigma.all <- vcov_summary(cav_MMBYM_inla_pc)

sigma_post_pc <- as.data.frame(sigma.all$var) %>% 
  dplyr::transmute(Year = c("Var_2021", "Var_2022", "Var_2023", "Var_2024"),
                   Mean = round(.data$mean, 3),
                   Q_0.025 = round(.data$quant0.025,3),
                   Median = round(.data$quant0.5,4),
                   Q_0.975 = round(.data$quant0.975,3))
rownames(sigma_post_pc) <- NULL

rho_post_pc<- as.data.frame(sigma.all$cor) %>% 
  dplyr::transmute(Year = c("Corr_2021/22", "Corr_2021/23", "Corr_2021/24", 
                            "Corr_2022/23", "Corr_2022/24", "Corr_2023/24"),
                   Mean = round(.data$mean, 3),
                   Q_0.025 = round(.data$quant0.025,3),
                   Median = round(.data$quant0.5,4),
                   Q_0.975 = round(.data$quant0.975,3))
rownames(rho_post_pc) <- NULL

sigma_all_post_pc <- dplyr::bind_rows(sigma_post_pc, rho_post_pc)

DT::datatable(sigma_all_post_pc)
```
...
## Possible development directions
  ...
  **Intepretation** and modelling:
  ...
  - Separate **factors** driving the two processes of **violence occurring** and **help seeking** 
    ...
    $\longrightarrow$ allocate **covariates** between two **separate** predictors: i.e. **Poisson--Logistic** regression [@Pogit], [@PogitVAW]

  Statistical **methodology**:
  
  - More insight on **PC prior** behaviour in PCAR and LCAR models $\longrightarrow$ **simulation** analysis

  - Extend the **PC-prior** approach to the **correlation** matrix [@PCgraphical]
