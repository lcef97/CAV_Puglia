...
$$
  ...
  - where: $\Sigma$: $4 \times 4$ covariance matrix;  $D$: Graph **degree** matrix;
  $W$: Graph **neighbourhood** matrix; $L:=D-W$: Graph **Laplacian** matrix
  
  - $+$ Easy to **fit** and **interpret**
  
  - $-$ implicitly assumes **spatial autocorrelation** equal to 1:
  ICAR models are sometimes too **smooth** to adequately fit data
...
## M-models
  ...
  - $M$ matrix: generic **full** rank matrix, in our application of size $k \times k$
    
  - We compare three **standard** models:
...
$$
Z \mid \Sigma \sim \mathcal{N}_{kn} \left( 0, \,
(M^\top \otimes I_n) (I_k \otimes D - A \otimes W)^{-1} (M \otimes I_n)
\right)
$$
  
  - $A := \mathrm{diag}(\alpha_1, ... \alpha_k)$: **autoregressive** parameter
...
$$
Z \mid \Sigma \sim \mathcal{N}_{kn} \left( 0, \,
(M^\top \otimes I_n) (\Lambda \otimes L + (I_k - \Lambda) \otimes I_n)^{-1} (M \otimes I_n)
\right)
$$
  
  - $\Lambda := \mathrm{diag}(\lambda_1, ... \lambda_k)$: **precision mixing** parameter
...
## Models(4): Besag-York_Mollié (BYM)
 $$
 Z \mid \Sigma \sim \mathcal{N}_{kn} \left( 0, \,
(M^\top \otimes I_n) \, \left[\Phi \otimes L^+ + (I_k - \Phi) \otimes I_n \right] \, (M \otimes I_n)
\right)
 $$
