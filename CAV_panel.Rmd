...
\begin{equation}
z \mid \Sigma \sim N \left( 0, \Sigma \otimes (D - W)^{+} \right)
\label{eq:icar_joint}
\end{equation}
...
\begin{equation}
z_i \mid z_{-i} \sim N \left( \sum_{j=1}^n \rho \frac{w_{ij}}{d_i} z_j \,, \frac{1}{d_i} \Lambda^{-1}\right)
\label{eq:pcar_loc}
\end{equation}
...
Where: 
  ...
  - $u \sim N(0, I_p \otimes L_\mathrm{scaled})$ is an independent multivariate ICAR process whose precision matrix is scaled in order that the geometric mean of marginal variances (i.e. the diagonal entries of $L^{+}$) is one
...
We therefore attempt at implementing a Poisson - logistic model [@Pogit] to explicitly account for under-reporting or, to be more precise, to model the propensity to report violence by turning to AVCs or not.
...
Here, we show the \texttt{R} code we propose to achieve this. 
