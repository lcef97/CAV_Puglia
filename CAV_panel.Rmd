...
The drawback, however, is that we implicitly assume a deterministic spatial autocorrelation coefficient equal to 1 each year. When the autocorrelation is weak, setting an ICAR prior may be a form of misspecification.

A generalisation of this model is the PCAR (proper CAR), which introduces an autocorrelation parameter $\rho = (\rho_{2021}, \rho_{2022}, \rho_{2023})$.

For the sake of notation, $z$ can be conveniently parametrised as $z := U M$, with $M:/ M^{\top}M = \Sigma$, and $U = (U_{2021}^{\top}, U_{2022}^{\top}, U_{2023}^{\top})^{\top}$, for $t = 2021, 2022, 2023$ $U_t \sim N(0, (D - \rho_t W)^{-1})$ and $U_t \bot U_s$ for $t \neq s$.

This modelling framework, generalising univiariate autoregressions, goes under the name of M-models [@Mmodels]. The joint prior for the M-PCAR is:

$z \mid \rho, \Sigma \sim  N \left( 0,  (M^{\top} \otimes I_n)\mathrm{Bdiag}(D - \rho_t W)(M \otimes I_n) \right)$

\begin{equation}
z \mid \rho, \Sigma \left(  (M^{\top} \otimes I_n)(\mathrm{Bdiag}(D - \rho_t W))(M \otimes I_n) \right)
\label{eq:pcar_joint}
\end{equation}

Where $\mathrm{Bdiag}(A_i)$ denotes a block-diagonal matrix with blocks given by $A_i$ for an appropriate set of indices.
The R code for implementing M-models is based on the script of [@INLAMSM], which has been extended to fit also the LCAR and BYM models (vd. infra)
```{r M model silent, echo = FALSE}
...
```{r MmodPCAR run, echo = F, output= F, warning = F, message =F}
cav_MmodPCAR_inla <- inla(
  N_ACC ~ 1 +TEP_th + ELI + PGR + UIS + ELL + PDI + ER+ 
        f(ID, model = inla.Mmodel(k = 3, W = W_con,  alpha.min = 0,alpha.max = 1, Qmod = "PCAR")),
    offset = log(nn),
    family =  "poisson", data =dd,
  #inla.mode = "classic", control.inla = list(strategy = "laplace", int.strategy = "grid"),
  num.threads = 1, control.compute = list(internal.opt = F, cpo = T,  waic = T, config = T), 
...
We show posterior summaries of the autocorrelation parameter. As it can be seen, the spatial component of the first two years is almost randomly distributed, while spatial autocorrelation is very strong for 2023.

```{r rho pcar, echo = FALSE, fig.height = 3, fig.cap = "Posterior marginal of the PCAR autocorrelation parameter", results = 'asis'}

rho_marg_pcar <- lapply(cav_MmodPCAR_inla$marginals.hyperpar[c(1,2,3)], function(f){
  inla.tmarginal(fun = function(X) 1/(1 + exp(-X)), marginal = f)
})
names(rho_marg_pcar) <- c("rho1", "rho2", "rho3")

rholimX <-range(do.call(c, lapply(rho_marg_pcar, FUN = function(x) x[,1])))
rholimY <-range(do.call(c, lapply(rho_marg_pcar, FUN = function(x) x[,2])))

rhomarg <- as.data.frame(do.call(cbind, rho_marg_pcar))
names(rhomarg) <- paste0(rep(names(rho_marg_pcar), each=2), c("__X", "__Y"))
names(rhomarg) <- 
  gsub("1__", "_._2021__", gsub("2__", "_._2022__", gsub("3__", "_._2023__",
                                                       names(rhomarg))))

rhomarg_long <- tidyr::pivot_longer(rhomarg, cols = c(1:ncol(rhomarg)),
...
ggplot2::ggplot(rhomarg_long, ggplot2::aes(x = .data$X, y = .data$Y, color = .data$Year)) +
  ggplot2::geom_line(size = 0.7) +
  ggplot2::geom_vline(xintercept = 0) +
  ggplot2::coord_cartesian(xlim = rholimX, ylim = rholimY) +
  ggplot2::labs(
    #title = "Posterior Marginals of Covariates Effects",
    x = expression(rho),
    y = expression(pi(rho ~ "|" ~ y)),
    color = "Autoregressive parameter") +
  ggplot2::theme_classic() 
...
xtable::xtable(rho_summary_pcar, digits = 3)
...
As an alternative to relax the hypothesis of spatial autocorrelation $=1$, we also test the Leroux autoregressive model [@Leroux], LCAR hereinafter. In this case, the joint prior for $z$ is

\begin{equation}
z \mid \xi, \Sigma \sim  N \left( 0,  (M^{\top} \otimes I_n)\mathrm{Bdiag}[\xi_t(D - W) + (1-\xi_t)I_n](M \otimes I_n) \right)
\label{eq:lcar_joint}
\end{equation}

With notation alike equation \ref{eq:pcar_joint}. $\xi \in [0, 1]^3$ is the mixing parameter.

```{r MmodLCAR run, echo = F, output= F, warning = F, message =F}
cav_MmodLCAR_inla <- inla(
  N_ACC ~ 1 +TEP_th + ELI + PGR + UIS + ELL + PDI + ER+ 
        f(ID, model = inla.Mmodel(k = 3, W = W_con,  alpha.min = 0,alpha.max = 1, Qmod = "LCAR")),
    offset = log(nn),
    family =  "poisson", data =dd,
  #inla.mode = "classic", control.inla = list(strategy = "laplace", int.strategy = "grid"),
  num.threads = 1, control.compute = list(internal.opt = F, cpo = T,  waic = T, config = T), 
...
```{r rho LCAR, echo = FALSE, fig.height = 3, fig.cap = "Posterior marginal of the LCAR autocorrelation parameter", results = 'asis'}

rho_marg_lcar <- lapply(cav_MmodLCAR_inla$marginals.hyperpar[c(1,2,3)], function(f){
  inla.tmarginal(fun = function(X) 1/(1 + exp(-X)), marginal = f)
})
names(rho_marg_lcar) <- c("rho1", "rho2", "rho3")

rholimX <-range(do.call(c, lapply(rho_marg_lcar, FUN = function(x) x[,1])))
rholimY <-range(do.call(c, lapply(rho_marg_lcar, FUN = function(x) x[,2])))

rhomarg <- as.data.frame(do.call(cbind, rho_marg_lcar))
names(rhomarg) <- paste0(rep(names(rho_marg_lcar), each=2), c("__X", "__Y"))
names(rhomarg) <- 
  gsub("1__", "_._2021__", gsub("2__", "_._2022__", gsub("3__", "_._2023__",
                                                       names(rhomarg))))
...
ggplot2::ggplot(rhomarg_long, ggplot2::aes(x = .data$X, y = .data$Y, color = .data$Year)) +
  ...
  ggplot2::labs(
    #title = "Posterior Marginals of Covariates Effects",
    x = expression(xi),
    y = expression(pi(xi ~ "|" ~ y)),
    color = "Autoregressive parameter") +
  ggplot2::theme_classic() 

rho_summary_lcar <- data.frame(do.call(rbind, lapply(
  rho_marg_lcar, function(x) unlist(inla.zmarginal(x, silent = TRUE))))) %>% 
  dplyr::select(1,2,3,5,7)

xtable::xtable(rho_summary_lcar, digits = 3)
...
#### R code for the M-model

```{r  M model displayed, eval = FALSE, echo = TRUE}
inla.rgeneric.Mmodel <- 
  function (cmd = c("graph", "Q", "mu", "initial", "log.norm.const", 
                    "log.prior", "quit"), theta = NULL) {
    envir <- parent.env(environment())
